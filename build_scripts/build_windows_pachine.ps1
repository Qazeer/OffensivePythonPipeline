...
Set-Location C:\host_build\Pachine\

# Create standalone binaries.
C:\Python\python.exe -m PyInstaller --specpath $env:temp\spec --workpath $env:temp\build --distpath $env:temp\out --clean -F C:\host_build\Pachine\pachine.py

# Export the compiled binaries.
Move-Item -Force $env:temp\out\pachine.exe C:\host_build\pachine_windows.exe
