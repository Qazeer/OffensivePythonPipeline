...
cp -r /host_build/smartbrute /smartbrute && cd /smartbrute
pip install -r requirements.txt

# Fix exit by importing sys.
sed -i 's/import os/import os\nfrom sys import exit\n/' /smartbrute/smartbrute.py

# Create standalone executables
pyinstaller --specpath /tmp/spec --workpath /tmp/build --distpath /tmp/out --clean -F /smartbrute/smartbrute.py

# Export the compiled binaries
mv /tmp/out/smartbrute /host_build/smartbrute_linux