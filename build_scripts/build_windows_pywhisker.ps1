...
Set-Location C:\host_build\pywhisker\
C:\Python\python.exe -m pip install -r C:\host_build\pywhisker\requirements.txt

# Create standalone binaries.
C:\Python\python.exe -m PyInstaller --specpath $env:temp\spec --workpath $env:temp\build --distpath $env:temp\out --clean -F C:\host_build\pywhisker\pywhisker.py

# Export the compiled binaries.
Move-Item -Force $env:temp\out\pywhisker.exe C:\host_build\pywhisker_windows.exe
